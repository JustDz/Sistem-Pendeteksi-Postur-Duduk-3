...
const DashboardMonitoring: React.FC = () => {
  const [latestSession, setLatestSession] = useState<StreamingSession | null>(
    null
  );
    ...
    const fetchStreamingData = async () => {
      try {
        const response = await fetch(
          "http://localhost:8080/get_streaming_history"
        );
  ...
  useEffect(() => {
    const fetchTimeSeriesData = async () => {
      try {
        const response = await fetch(
          "http://localhost:8080/get_latest_session_data"
        );
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-700 p-3 rounded">
              ...
              <p className="text-lg font-semibold">
                {formatPercentage(stats.sit.good)}
              </p>
            </div>
            <div className="bg-gray-700 p-3 rounded">
              <p className="text-sm">Buruk</p>
              <p className="text-lg font-semibold">
                {formatPercentage(stats.sit.bad)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-gray-700 p-3 rounded">
              ...
              <p className="text-lg font-semibold">
                {formatPercentage(stats.spine.normal)}
              </p>
            </div>
            <div className="bg-gray-700 p-3 rounded">
              <p className="text-sm">Lordosis</p>
              <p className="text-lg font-semibold">
                {formatPercentage(stats.spine.lordosis)}
              </p>
            </div>
            <div className="bg-gray-700 p-3 rounded">
              <p className="text-sm">Kifosis</p>
              <p className="text-lg font-semibold">
                {formatPercentage(stats.spine.kifosis)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-3 rounded shadow-lg border border-gray-700">
          ...
          <p className="text-sm">
            ...
            <span className="capitalize">
              {payload[0].payload.diagnosisSit}
            </span>
          </p>
          <p className="text-sm">
            <span className="text-green-400">Postur Punggung: </span>
            <span className="capitalize">
              {payload[0].payload.diagnosisSpine}
            </span>
          </p>
        </div>
      );
  ...
  return (
    <section
      id="Monitoring"
      className="bg-body-clr text-white min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal>
          <div className=" flex items-center justify-center mb-8">
            <h1 className="mt-6 text-4xl md:text-6xl font-bold text-center">
              Monitoring Postur Tubuh
            </h1>
            <button
              onClick={toggleModal}
              className="mt-6 ml-4 bg-body-clr p-2 rounded-full hover:bg-body-clr focus:outline-none">
              <i className="bx bx-info-circle text-3xl text-yellow-300 hover:text-yellow-600"></i>
            </button>
          </div>
        </Reveal>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-gray-800 rounded-lg p-6 shadow-lg max-w-4xl w-full">
              ...
              <p className="text-white text-md text-justify mb-4">
                Sistem ini memonitor postur tubuh Anda selama sesi berlangsung
                dengan menggunakan data real-time. Data tersebut dianalisis
                untuk mendeteksi posisi duduk dan kondisi tulang belakang Anda.
                Dalam halaman monitoring ini, Anda akan menemukan:
              </p>
              <ol className="text-left list-decimal list-inside text-white mb-4 space-y-5">
                <li>
                  <strong>Sesi terakhir pengecekan postur tubuh Anda</strong>{" "}
                  yang mencakup:
                  <ul className="ml-6 list-disc list-inside text-white mt-2 text-sm">
                    <li>
                      Informasi sesi tentang Waktu mulai, waktu selesai, dan
                      durasi sesi.
                    </li>
                    <li>
                      Statistik persentase posisi duduk (baik atau buruk) serta
                      kondisi tulang belakang (normal, lordosis, atau kifosis).
                    </li>
                    <li>
                      Posisi duduk dan tulang punggung yang paling sering
                      terlihat selama sesi berlangsung.
                    </li>
                  </ul>
                </li>

                <li>
                  <strong>Riwayat sesi</strong> yang telah Anda lakukan,
                  mencakup:
                  <ul className="ml-6 list-disc list-inside text-white mt-2 text-sm">
                    <li>Tanggal dan jam sesi dimulai.</li>
                    <li>Total waktu yang digunakan dalam sesi.</li>
                    <li>Postur yang paling sering terlihat selama sesi.</li>
                  </ul>
                </li>

                <li>
                  <strong>Grafik Monitoring Postur</strong> yang menampilkan
                  frekuensi postur duduk dan posisi tulang belakang. Yang akan
                  berisi:
                  <ul className="list-disc list-inside ml-6 mt-2 text-sm">
                    <li>Waktu pengecekan</li>
                    <li>Informasi mengenai postur duduk</li>
                    <li>Informasi mengenai posisi tulang punggung</li>
                  </ul>
                </li>
                <li>
                  <strong>Rangkuman Statistik</strong>, meliputi:
                  <ul className="list-disc list-inside ml-6 text-sm mt-2">
                    <li>Total sesi yang telah dilakukan.</li>
                    <li>Total waktu sesi yang tercatat.</li>
                    <li>Rata-rata durasi streaming pengecekan.</li>
                    <li>Waktu sesi terakhir dilakukan.</li>
                  </ul>
                </li>
              </ol>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Status Panel */}
          <Reveal>
            <div className="min-h-[612px] bg-gray-800 rounded-lg p-6 shadow-lg">
                    ...
                    {formatDuration({
                      hours: Math.floor(
                        sessionHistory.reduce(
                          (acc, session) =>
                            acc + (session.duration?.total_seconds || 0),
                          0
                        ) / 3600
                      ),
                      minutes: Math.floor(
